...
const HANDLERS = {
  ...
  'auth-email-check': () => import('../../server/lib/app-auth-email-check.js'),
  'support-submit': () => import('../../server/lib/app-support-submit.js'),
};
...
export default async function handler(req, res) {
  ...
  if (!loader) {
    return json(res, 400, { error: 'Unknown app endpoint kind.', expected: ['public-config','runtime-config','health','usage-check','import-capacity','dev-tools','durable-sync','auth-email-check','support-submit'] });
  }
