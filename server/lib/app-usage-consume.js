...
//
// The broken RPC path (consume_user_usage with wrong arg names) is removed.
// The JS fallback (getUsageRow + upsertUsageRow) is the only write path.
...
const ACTION_COSTS = new Map([
  ['tts', 2],
  ['book_import', 6],
  ['import', 6],
]);
const DEFAULT_PROTECTED_ACTION_COST = 2;
...
export default async function handler(req, res) {
  ...
  const cost = ACTION_COSTS.get(action) || DEFAULT_PROTECTED_ACTION_COST;
  ...
  if (!token) {
    ...
    // there is no durable row to write. Return denied for unauthenticated
    // consume attempts so the caller knows this was not recorded.
    return json(res, 200, {
  ...
  if (!user?.id) {
    // Token present but could not be resolved. Surface this explicitly rather
    // than falling to a guest path — the caller should know auth is the issue.
    return json(res, 200, {
      ok: false, allowed: false, action, cost,
      totalSpent: null, remaining: null, limit: usageDailyLimit,
