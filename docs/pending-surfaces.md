...
| Onboarding / reading settings → **voice selection preview** | Role-resolved one-word preview | — 1A lightweight preview only. Selecting Sara/Jenny/William/Davis in onboarding, or changing voice in reading settings, may utter `Hey`. Demo/Pro cloud selections use the existing `/api/ai?action=tts` voice-preview request with the selected provider voice ID; Basic/browser selections use browser speech. Preview does not consume usage, does not enter the Read Aloud queue, does not persist demo as entitlement state, and fails silently. Basic/browser contexts must not carry dead cloud selections into runtime playback. |
