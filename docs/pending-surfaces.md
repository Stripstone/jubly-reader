...
| Surface | What it does | Status |
|---|---|---|
| Dashboard → **Library grid** (renders on auth) | IndexedDB read + remote sync | ✅ In-surface pending remains the primary seam; delayed banner appears only if hydration noticeably stalls. Empty/import guidance is delayed by the same minimum empty-grace window and appears only through `#library-empty`; import/manage toolbar chrome is not a fallback while library truth is pending. |
