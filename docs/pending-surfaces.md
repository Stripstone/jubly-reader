...
| Reading view → `#shell-playback-indicator` | Shell-presented floating playback/support notice from runtime getter truth through `syncShellPlaybackControls()` | ◐ Hidden when playback is healthy. Floats centered above the bottom playback bar; shows voice-volume-off support copy, delayed cloud-restart copy after the no-flash threshold, and `Buffering…` with spinner only when runtime reports that speech has stopped and full-page promotion readiness is blocking continuation. Countdown may prepare full-page audio after clean prior-page engagement, or cheap Phase 1 audio after prior-page skips/uncertain engagement; this preparation remains silent unless playback later reaches a real wait. Device/audio unavailable copy is a settled banner, not inline playback-control text. Playback-start failure remains held until runtime exposes a retry-exhausted/start-error signal outside `tts.js`. |
...
- Windowed TTS promotion is commitment-based, not block-ordinal-based. Runtime may trigger full-page promotion after committed audible character progress through Phase 1, including during a long block 0; a tiny block 1 must not become accidental full-page entitlement.
- Autoplay countdown should prepare the next page silently: full-page when the prior page completed cleanly, cheap Phase 1 when prior skips/uncertain behavior make full-page spend unjustified.
