...
window.rcInteraction = (function () {
  ...
  return { pending, success, error, clear, clearAll, actions: actionPresets };
    ...
    function renderAnalyticsSurface() {
        ...
        if (sessionsEl) sessionsEl.textContent = String(metrics.sessionsCompleted || 0);
        if (ringEl) ringEl.style.setProperty('--goal-progress', `${Math.max(0, Math.min(100, Number(metrics.progressPct || 0)))}%`);
        if (copyEl) copyEl.textContent = metrics.progressPct >= 100 ? 'Daily goal complete.' : `${Math.max(0, goalMinutes - displayDaily)} min to go today.`;
        try {
    ...
    function initShellNav1E() {
        ...
        // History: shows #sh-history-surface, hides main sections
        ...
        if (sbHistory) {
            sbHistory.addEventListener('click', () => {
                ...
                try {
                    ...
                    document.querySelectorAll('#app-sidebar .sidebar-btn:not(.sh-secondary-btn)').forEach(b => b.classList.remove('active'));
