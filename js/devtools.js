window.rcDevTools = (function () {
  ...
  function renderPanel() {
    ...
    const clientHasActiveReading = !!(clientTarget && clientTarget.bookId && clientTarget.sourceType);
    const resolvedProgressBookId = clientHasActiveReading ? (clientTarget.bookId || client.selectors.bookId || latestProgress.book_id || '') : (latestProgress.book_id || clientTarget.bookId || client.selectors.bookId || '');
    const resolvedProgressChapter = clientHasActiveReading
      ? (clientTarget.chapterIndex != null && Number(clientTarget.chapterIndex) >= 0 ? clientTarget.chapterIndex : (latestProgress.chapter_id != null ? latestProgress.chapter_id : (client.selectors.chapterId || '')))
      : (latestProgress.chapter_id != null ? latestProgress.chapter_id : (clientTarget.chapterIndex != null && Number(clientTarget.chapterIndex) >= 0 ? clientTarget.chapterIndex : (client.selectors.chapterId || '')));
    const resolvedProgressPageIndex = clientHasActiveReading
      ? (clientTarget.pageIndex != null ? clientTarget.pageIndex : (latestProgress.last_page_index != null ? latestProgress.last_page_index : (clientRestore.currentPageIndex || 0)))
      : (latestProgress.last_page_index != null ? latestProgress.last_page_index : (clientTarget.pageIndex != null ? clientTarget.pageIndex : (clientRestore.currentPageIndex || 0)));
    const resolvedProgressPage = resolvedProgressPageIndex == null ? '' : (Math.max(0, Number(resolvedProgressPageIndex)) + 1);
    ...
    setValue('devProgressPage', resolvedProgressPage === '' ? '' : resolvedProgressPage);
    ...
    if (summaryEl) {
      ...
      const clientPage = Number.isFinite(Number(resolvedProgressPage)) ? Number(resolvedProgressPage) : '—';
